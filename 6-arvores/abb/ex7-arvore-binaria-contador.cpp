...
class BinaryTree {
  ...
  public:
    ...
    int getQtdOccurrencesOf(Data value);
...
int BinaryTree::getQtdOccurrencesOf(Data value) {
    Node* current = this->getNode(value);

    return current != NULL ? current->occurrence : 0;
}
...
int main() {
    ...
    for (unsigned i = 0; i < numberOfElements; i++) {
        cin >> input;
        if (bt.search(input)) {
            cout << input << " existe e foi cadastrado " << bt.getQtdOccurrencesOf(input)
                 << " vez(es)." << endl;
        } else {
    ...
    cin >> numberOfElements;
    for (unsigned i = 0, occurrencesOfRemoved; i < numberOfElements; i++) {
        cin >> input;
        cout << input << " foi cadastrado " << bt.getQtdOccurrencesOf(input)
             << " vez(es) e será removido." << endl;
        bt.remove(input);
        cout << "Agora ";
        occurrencesOfRemoved = bt.getQtdOccurrencesOf(input);
        if (occurrencesOfRemoved > 0) {
            cout << " há apenas " << occurrencesOfRemoved << " valores cadastrados."
                 << endl;
        } else {
            cout << " não há mais registros dele salvo." << endl;
        }
    }
